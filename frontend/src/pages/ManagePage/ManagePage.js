			...
			return (
				<>
					...
					<main className="container p-5" style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-around' }}>
						{!isObjEmpty(this.state.user) ? this.state.user.bag.map(p => <Card pokeid={p.poke_id} deleteFunc={this.delete} key={p.poke_name + Date.now() + Math.random()} name={p.poke_name} img={p.poke_img} hp={p.poke_hp} attack={p.poke_attack} defence={p.poke_defence} weight={p.poke_weight} />) : <></>}
					</main>
				</>
			)
		}
